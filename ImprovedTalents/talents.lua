...
local SV -- saved vars table (TALENTSVIEWER_SV)
local frame
local treeFrames = {}
local talentButtons = {}
local branchArrays = {}
...
-- Create a minimal checkbox (uses global template available in 1.12)
...
local function CreateMainFrame()
    ...
    -- expose checkbox so loader can apply saved state after ADDON_LOADED
    frame.scaleCheckbox = scaleCheckbox

    if SV and SV['TalentFrameSmall'] then frame:SetScale(0.8); scaleCheckbox:SetChecked(1) end
...
local function CreateTreeFrames()
    ...
    for i = 1, 3 do
        ...
        -- background pieces use Blizzard talent textures (GetTalentTabInfo provides fileName)
        local bgTopLeft = treeFrame:CreateTexture(nil, 'BACKGROUND')
        bgTopLeft:SetWidth(200); bgTopLeft:SetHeight(300); bgTopLeft:SetPoint('TOPLEFT', treeFrame, 'TOPLEFT', 20, -30)
        local bgTopRight = treeFrame:CreateTexture(nil, 'BACKGROUND')
        bgTopRight:SetWidth(100); bgTopRight:SetHeight(300); bgTopRight:SetPoint('TOPRIGHT', treeFrame, 'TOPRIGHT', 20, -30)
        local bgBottomLeft = treeFrame:CreateTexture(nil, 'BACKGROUND')
        bgBottomLeft:SetWidth(200); bgBottomLeft:SetHeight(200); bgBottomLeft:SetPoint('BOTTOMLEFT', treeFrame, 'BOTTOMLEFT', 20, -30)
        local bgBottomRight = treeFrame:CreateTexture(nil, 'BACKGROUND')
        bgBottomRight:SetWidth(100); bgBottomRight:SetHeight(200); bgBottomRight:SetPoint('BOTTOMRIGHT', treeFrame, 'BOTTOMRIGHT', 20, -30)
        ...
        borderLeft:SetWidth(4); borderLeft:SetHeight(420)
        ...
        borderRight:SetWidth(4); borderRight:SetHeight(420)
        ...
        treeFrames[i] = { frame = treeFrame, header = header, pointsText = pointsText, branchFrame = branchFrame, arrowFrame = arrowFrame }
    end
end
...
local function CreateTalentButton(tabIndex, talentIndex, tier, column)
    ...
    button.tabIndex = tabIndex; button.talentIndex = talentIndex
...
local function ResetBranches(tabIndex)
    for tier = 1, 8 do
        for col = 1, 4 do
            ...
            node.id = nil; node.up = 0; node.down = 0; node.left = 0; node.right = 0; node.rightArrow = 0; node.leftArrow = 0; node.topArrow = 0
        end
    end
    for i = 1, table.getn(branchTextures[tabIndex]) do branchTextures[tabIndex][i]:Hide() end
    for i = 1, table.getn(arrowTextures[tabIndex]) do arrowTextures[tabIndex][i]:Hide() end
end
        ...
        for talentIndex = 1, numTalents do
            ...
            if button then
                ...
                if talentName then
                    ...
                    if UnitCharacterPoints('player') <= 0 and rank == 0 then forceDesaturated = 1 else forceDesaturated = nil end
                    local tierUnlocked2
                    if (tier - 1) * 5 <= pointsSpent then tierUnlocked2 = 1 else tierUnlocked2 = nil end
...
local function ToggleFrame()
    ...
    if frame:IsVisible() then frame:Hide() else frame:Show(); Update() end
end
...
-- Expose global toggle used by keybinds
...
-- Hook micro button update to reflect talent frame state
if UpdateMicroButtons then
    ...
    UpdateMicroButtons = function()
        ...
        if DFRL and DFRL.menuframe and DFRL.menuframe:IsVisible() and MainMenuMicroButton then MainMenuMicroButton:SetButtonState('PUSHED', 1) end
    end
end

-- Wait for savedvars to be ready
local loader = CreateFrame('Frame')
loader:RegisterEvent('ADDON_LOADED')
loader:SetScript('OnEvent', function()
    -- In 1.12 the event system does not pass arguments to the handler;
    -- use the global arg1 instead.
    if arg1 ~= 'ImprovedTalents' then return end

    -- Ensure global savedvar exists and reference it
    TALENTSVIEWER_SV = TALENTSVIEWER_SV or {}
    SV = TALENTSVIEWER_SV

    -- The main frame is created lazily on first open, so it does not exist
    -- yet here.  We store nothing extra; CreateMainFrame already reads SV
    -- at the moment it runs, so the saved scale will be applied then.
